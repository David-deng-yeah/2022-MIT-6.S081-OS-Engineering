...
//递归释放一个内核页表中的所有mapping，但是不释放其指向的物理页
void 
kvm_free_kernelpgtbl(pagetable_t pagetable)
{
  ...
  kfree((void*)pagetable);//释放当前页表所占的空间
}


//这个函数可以将内核中需要的direct mapping 添加到页表 pgtbl中
...
kvminit()
{
  // 还是需要有全局的内核页表，用于内核boot过程，以及无进程在运行时使用
  kernel_pagetable = kvminit_newpgtbl();
  /*先注释掉
  kernel_pagetable = (fpagetable_t) kalloc();
...
{
  //一级页表有2^9=512个页表项(pte)
  for(int i = 0; i < 512; i++){
    pte_t pte = pagetable[i];
    if(pte & PTE_V){//如果页表项有效?
      // the pte point to the lower-level page table
